...
RSpec.feature 'Navigation bar', :devise do
	...
	let(:developer2) {FactoryBot.create(:user, name: 'Mock developer1', email: 'developer2@ex.com', role: 'developer', confirmed_at: Time.now.utc)}
	...
	scenario "a user can view other user's profile, but 'Edit Profile' btn not displayed" do
		...
		visit user_path(developer1)

		expect(current_path).to eq "#{user_path(developer1)}"
		expect(page).to_not have_link("Edit Profile", edit_user_path(developer1))
	end
